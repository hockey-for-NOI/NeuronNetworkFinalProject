...
class Model_rearr:
    def __init__(self,
        ...
        _ = tf.matmul(tf.reshape(_, [-1, 392]), weight_variable([392, 10])) + bias_variable([10])
        logits = _
        

        self.loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y_, logits=logits)) + \
                    tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y_rearr_, logits=_rearr))

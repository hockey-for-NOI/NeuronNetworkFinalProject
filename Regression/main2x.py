...
def work(n_split):
    ...
    X_train_all, X_test_all, y_train_all, y_test_all = load_data(n_split)

    mlp_model = Model(True)

    model_pref = "CNN_std_{}split".format(n_split)
    train_dir = "{}_CNN_std_{}split".format(FLAGS.train_dir_pref, n_split)
